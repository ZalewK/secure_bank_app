...
class TransferForm(FlaskForm):
    amount = IntegerField('Amount', validators=[DataRequired(message="Kwota przelewu musi być liczbą."), Length(1, 50), NumberRange(min=1, max=1000000), Regexp(safe_transaction_amount, message=safe_transaction_amount_message)])
...
class ChangePasswordForm(FlaskForm):
    current_password = PasswordField('Aktualne hasło', validators=[DataRequired(), Length(4, 12), Regexp(safe_password, message=safe_password_message)])
    new_password = PasswordField('Nowe hasło', validators=[DataRequired(), Length(4, 12), Regexp(safe_password, message=safe_password_message)])
    confirm_password = PasswordField('Potwierdź nowe hasło', validators=[DataRequired(), Length(4, 12), Regexp(safe_password, message=safe_password_message), EqualTo('new_password', message="Hasła muszą się zgadzać.")])
